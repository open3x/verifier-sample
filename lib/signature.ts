...
export async function createSignature({ address, result, counter }: { address: Address, result: boolean; counter: bigint }): Promise<Hex> {
  const encodedData = encodeAbiParameters(parseAbiParameters("address, bool, uint256"), [address, result, counter]);
