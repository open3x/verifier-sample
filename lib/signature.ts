...
export async function createSignature({ address, mint_eligibility, data }: { address: Address, mint_eligibility: boolean; data?: string }): Promise<Hex> {
  const encodedData = encodeAbiParameters(parseAbiParameters("address, bool, bytes32"), [address, mint_eligibility, toHex(data || 0, { size: 32 })]);
